...
export default function VideoToQuiz() {
  ...
  const handleTranscriptFetch = async () => {
    setLoading(true);
    try {
      const response = await axios.post('/api/transcript', { videoUrl: String(videoUrl) }, {
  ...
  return (
    <div className="w-full min-h-screen bg-background text-foreground">
      ...
      <main>
        <section className="container mx-auto px-4 md:px-6 py-12 md:py-24 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="max-w-xl space-y-4">
            ...
            <div className="flex items-center gap-2 w-full max-w-md">
              <Input type="text" placeholder="Enter YouTube video link" className="flex-1" value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} />
        ...
        {quiz && !viewSavedQuiz && (
          <form onSubmit={handleQuizSubmit} className="container mx-auto px-4 md:px-6 py-12 md:py-24">
            <h2 className="text-3xl md:text-4xl font-bold text-center mb-8">Generated Quiz</h2>
            {quiz.map((q, index) => (
